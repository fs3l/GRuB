...
TXLogFile = path_prefix + 'TX_always_replicate_' + file_suffix
...
TXLogFile += '_' + Contract_Type + '_Always_Replicate'
SC.get_contract_instance('./contracts_motivation_v2/memorizing/Always_Replicate.sol', TXLogFile)
...
if test_type == 1:
    for i in range(test_times):
        ...
        for j in range(write_times):
            SC.send_transactions(Contract_Type, 1, [operate_range[0], operate_range[-1], values], range_size, 'Write')
     
        # read one time
        print('OnChain read:', onChainReadKeys)
        SC.send_transactions(Contract_Type, 3, [operate_range[0], operate_range[-1]], range_size, 'OnChainRead')
