...
SC.get_contract_instance('./contracts/StableCoin_GRuB.sol', TXLogFile)
...
if test_type == 1:
    ...
    for j in range(read_times):
        if j > K: 
            SC.send_transactions(Contract_Type, 3, [read_keys, [], False, [], [], depth], 1, 'Deposit_ON')
        elif j == K: 
            SC.send_transactions(Contract_Type, 3, [read_keys, read_values, True, indices, proof, depth], 1, 'Deposit_OFFR')
        else:
            SC.send_transactions(Contract_Type, 3, [read_keys, read_values, False, indices, proof, depth], 1, 'Deposit_NR')

    for j in range(read_times):
        SC.send_transactions(Contract_Type, 1, [[read_keys], root], batch_size, 'Poke')
    SC.send_transactions(Contract_Type, 3, [read_keys, read_values, False, indices, proof, depth], 1, 'Deposit_NR')
