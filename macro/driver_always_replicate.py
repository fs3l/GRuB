...
loading_range=200
TXLogFile += '_Always_Replicate_' + str(K)
Contract_type = 'GRuB_Range_Query'
SC.get_contract_instance('./contracts/memorizing/Always_Replicate.sol', TXLogFile)
...
if test_type == 1:
    ...
    for i in range(len(Batches)):
        ...
        if  batch[0][0] == 'READ' or batch[0][0] == 'SCAN':
            ...
            #SC.send_transactions(Contract_type, 1, [indices[0], indices[-1], values], len(batch), 'W_'+str(i))
            SC.send_transactions(Contract_type, 4, [indices, values], len(batch), 'W_'+str(i))
